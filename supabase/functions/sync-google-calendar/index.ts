  ...
  try {
    ...
    if (!expiry || expiry <= Date.now() + 60_000) {
      ...
      if (updateError) {
        ...
        return new Response(JSON.stringify({ error: 'Failed to update token' }), { status: 500, headers: corsHeaders })
      }

      activeToken = { ...activeToken, access_token: refreshed.access_token, refresh_token: refreshToken, expiry_date: newExpiry }
    }
