...
export function AnalyticsPage() {
  ...
  const getCategoryData = (): CategoryData[] => {
    ...
    expenses
      .filter(expense => expense.amount > 0) // 支出のみ（収入を除外）
      .forEach(expense => {
  ...
  if (loading) {
    return (
      <div className="space-y-6 rounded-3xl bg-slate-50/80 border border-slate-200 p-6 sm:p-8 shadow-[0_20px_60px_rgba(15,23,42,0.08)]">
  ...
  if (timeRange === 'current') {
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 mb-2">今月の支出</h1>
          ...
          <div>
            <select
              ...
              className="glass-input px-4 py-2 sm:w-auto"
            >
        ...
        {!hasExpenses && budget <= 0 ? (
          // 記録がない場合
          <div className="rounded-2xl bg-white border border-slate-200/70 p-8 sm:p-12 text-center shadow-sm">
          ...
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
              <div className="glass-card">
                <p className="text-slate-600 font-semibold text-sm sm:text-base mb-2">今月の予算</p>
                <p className="text-2xl font-bold text-slate-900">¥{budget.toLocaleString()}</p>
              </div>

              <div className="glass-card">
                <div className="flex items-center gap-4">
                  <p className="text-slate-600 font-semibold text-sm sm:text-base mb-2">実支出</p>
              ...
              <div className="glass-card">
                <p className="text-slate-600 font-semibold text-sm sm:text-base mb-2">残り</p>
                <p
                  className={`text-2xl font-bold ${remaining >= 0 ? 'text-emerald-600' : 'text-rose-500'}`}
                >
            ...
            <div>
              <div className="flex items-center space-x-3">
                <h2 className="text-xl sm:text-xl font-bold text-slate-900 mb-2">予算の使用率</h2>
              </div>
              <div className="glass-card">
                ...
                <div className="relative">
                  ...
                  {budgetPercentage > 100 && (
                    <div className="absolute top-1/2 -translate-y-1/2 right-0 -mr-8">
                      <span className="text-rose-500 font-semibold text-sm">超過</span>
                    </div>
                  )}
                </div>
            ...
            {(categoryData.length > 0 || expenses.length > 0) && (
              <div>
                <div className="flex items-center space-x-3">
                  <h2 className="text-xl sm:text-xl font-bold text-slate-900 mb-2">
                      ...
                      <div className="flex flex-col lg:flex-row gap-6">
                        ...
                        <div className="flex-1 space-y-2">
                          {categoryData.map((category, index) => (
                            <div
                              key={index}
                              className="flex items-center p-2 sm:p-4 bg-slate-50 hover:bg-white rounded-lg sm:rounded-xl border border-slate-200/70 transition-all"
                            >
                      ...
                      <div className="space-y-2">
                        {(() => {
                          ...
                          return sortedExpenses.map(expense => (
                            <div
                              key={expense.id}
                              className="flex items-center justify-between p-3 sm:p-4 bg-slate-50 hover:bg-white rounded-lg sm:rounded-xl border border-slate-200/70 transition-all"
                            >
  ...
  // 全期間選択時の新しいPC向けレイアウト
  return (
    <div className="space-y-8 rounded-3xlsm:p-8">
      {/* Page Header */}
      <div className="flex flex-col md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 mb-2">全期間</h1>
          <p className="text-slate-500">支出パターンと貯金の進捗を詳しく分析しましょう</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div>
          <div className="flex items-center space-x-3 mb-2">
            <h2 className="text-xl font-bold text-slate-900">貯蓄目標の進捗</h2>
          </div>
          <div className="glass-card">
            {getSavingsProgressData() ? (
                ...
                return (
                  <div className="space-y-4">
                    <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-2">
                      <div>
                        <p className="text-slate-500 text-sm">目標</p>
                        <p className="text-2xl font-bold text-slate-900">{sp.title || '未設定'}</p>
                      </div>
                      <div className="text-right">
                        <p className="text-slate-500 text-sm">目標金額</p>
                        <p className="text-2xl font-bold text-slate-900">
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center justify-between">
                        <span className="text-slate-500 text-sm">今日の理想到達額（ペース）</span>
                      ...
                      <div className="flex items-center justify-between">
                        <span className="text-slate-500 text-sm">実績（全期間・余剰の累計）</span>
                      ...
                      <div className="flex items-center justify-between">
                        <div
                          className={`${sp.aheadAmount >= 0 ? 'text-emerald-600' : 'text-rose-500'} text-right font-medium`}
                        >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                      <div className="rounded-xl bg-slate-50 border border-slate-200/70 p-3">
                        <p className="text-slate-500 text-xs">残り金額</p>
                        <p className="text-lg font-semibold text-slate-900">
                          ¥{Math.ceil(sp.remaining).toLocaleString()}
                        </p>
                      </div>
                      <div className="rounded-xl bg-slate-50 border border-slate-200/70 p-3">
                        <p className="text-slate-500 text-xs">目標日</p>
                        <p className="text-lg font-semibold text-slate-900">
                          {format(new Date(sp.targetDate), 'yyyy/MM/dd')}
                        </p>
                      </div>
                      <div className="rounded-xl bg-slate-50 border border-slate-200/70 p-3">
                        <p className="text-slate-500 text-xs">分析期間</p>
                        <p className="text-lg font-semibold text-slate-900">
              ...
              <div className="text-center text-slate-500">貯蓄目標が未設定です。</div>
            )}
          </div>
        </div>
        {/* 月別支出推移 */}
                  ...
                  return (
                    <div key={index} className="space-y-2">
                      <div className="flex justify-between items-center">
                        <span className="font-medium text-slate-900">{data.month}</span>
                        <div className="flex items-center space-x-4">
                          <span className="text-sm text-slate-600">
                            予算: ¥{data.budget.toLocaleString()}
                          </span>
                          <span
                            className={`font-semibold ${isOverBudget ? 'text-rose-500' : 'text-slate-900'}`}
                          >
                      ...
                      <div className="flex justify-between text-xs text-slate-500">
                        <span>0%</span>
                        <span
                          className={isOverBudget ? 'text-rose-500 font-medium' : 'text-slate-600'}
                        >
      ...
      <div>
        <div className="flex items-center space-x-3 mb-2">
          <h2 className="text-xl font-bold text-slate-900">データ</h2>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2">
          <div className="glass-card">
            <p className="text-sm font-bold text-slate-500 mb-1">総支出</p>
            <p className="text-2xl font-semibold text-slate-900">
              ¥{totalExpenses.toLocaleString()}
            </p>
          </div>
          <div className="glass-card">
            <p className="text-sm font-bold text-slate-500 mb-1">支出記録数</p>
            <p className="text-2xl font-semibold text-slate-900">{expenses.length}件</p>
          </div>
          <div className="glass-card">
            <p className="text-sm font-bold text-slate-500 mb-1">平均月間支出</p>
            <p className="text-2xl font-semibold text-slate-900">
              ¥
          ...
          <div className="glass-card">
            <p className="text-sm font-bold text-slate-500 mb-1">目標達成率</p>
            <p className="text-2xl font-semibold text-slate-900">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-xl font-bold glass-text-strong">月毎の貯蓄額</h3>
            <span className="text-sm glass-text">開始〜今日</span>
          </div>
              ...
              return (
                <div className="w-full">
                  ...
                  <div className="relative h-64 w-full">
                    {/* Y-axis labels */}
                    <div className="absolute left-0 top-0 bottom-8 flex flex-col justify-between text-xs glass-text">
                    ...
                    {/* Grid lines and bars */}
                    <div className="absolute left-12 right-0 top-0 bottom-8 flex flex-col">
                      ...
                      <div className="relative flex-1">
                        ...
                        <div className="absolute inset-0 flex items-end justify-start gap-2 px-4">
                          {series.values.map((v, i) => {
                            const percentage = (Math.abs(v) / roundedMax) * 100;
                            ...
                            return (
                              ...
                              >
                                ...
                                {/* Tooltip on hover */}
                                <div className="absolute bottom-full mb-2 hidden group-hover:block w-max px-3 py-2 bg-gray-900/90   text-white text-xs rounded-lg shadow-xl z-20">
                      ...
                      <div className="flex items-start justify-start gap-2 px-4 pt-2">
                        {series.labels.map((label, i) => (
                          <div
                            key={i}
                            className="flex-1 text-center text-xs glass-text"
                            style={{ maxWidth: '40px', minWidth: '20px' }}
                          >
                            {label}
                          </div>
        ...
        <div>
          ...
          <div className="glass-card lg:col-span-2">
            {categoryData.length > 0 ? (
              <div className="flex flex-col lg:flex-row gap-6">
                      ...
                      <div key={index} className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-baseline gap-2">
                            <span className="font-semibold glass-text-strong text-sm">
                              {percentage.toFixed(0)}%
                            </span>
                            <span className="text-xs glass-text truncate">{category.category}</span>
                          </div>
                        </div>
                      </div>
                    );
                ...
                <div className="flex items-center justify-center lg:w-40">
                  <div className="relative w-32 h-32">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      <div className="text-center">
                        <div className="text-xs font-bold glass-text-strong">
                          ¥{categoryData.reduce((sum, cat) => sum + cat.amount, 0).toLocaleString()}
                        </div>
                        <div className="text-[8px] glass-text">合計</div>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="text-center glass-text py-10">データがありません。</div>
            )}
          </div>
        </div>
      </div>
