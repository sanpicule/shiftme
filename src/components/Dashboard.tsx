...
export function Dashboard({ onNavigate }: DashboardProps) {
  ...
  const { allExpenses, fixedExpenses, savingsGoals, loading, refetchData } = useData();
  ...
  if (isBeforeStartMonth) {
    ...
    weeklyBudget = Math.max(
      0,
      Math.floor(remainingBudget / Math.max(1, Math.ceil(remainingDays / 7))),
    );
  }
    ...
    <div className="space-y-4 md:space-y-8">
      ...
      <div className="relative">
        ...
        <div className="absolute inset-0 opacity-20 pointer-events-none">
          ...
          <div
            className="absolute bottom-0 right-0 w-48 h-48 bg-white/20 rounded-full translate-x-24 translate-y-24 glass-float"
            style={{ animationDelay: '1s' }}
          ></div>
          <div
            className="absolute top-1/2 left-1/2 w-24 h-24 bg-white/25 rounded-full -translate-x-12 -translate-y-12 glass-float"
            style={{ animationDelay: '2s' }}
          ></div>
        </div>

        {/* Details Toggle Button - Top Right */}
        <div className="absolute top-10 right-0 md:hidden z-10">
          <button
          ...
          {remainingBudget >= baseMonthlyBudget * 0.2 && (
            <div className="flex items-start space-x-2 md:space-x-3 p-3 bg-green-500/20 rounded-xl border border-green-400">
              ...
              <div>
                ...
                <p className="text-xs md:text-sm text-gray-800 text-shadow">
                  この調子で貯金目標を達成しましょう！
                </p>
              </div>
            </div>
            ...
            <div className="p-4 sm:p-6 h-[90vh] overflow-y-scroll glass-scrollbar">
              ...
              {(isAddingExpense || editingExpense) && (
                <div className="mb-6 p-6 glass-card">
                  ...
                  <form
                    onSubmit={handleSubmit(editingExpense ? updateExpense : addExpense)}
                    className="space-y-6"
                  >
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                      ...
                      <div>
                        <label className="block text-sm font-semibold glass-text mb-3">
                          金額（円）
                        </label>
                        <input
                          type="number"
              ...
              expenses.filter(
                ...
                <div className="space-y-4 overflow-auto">
                  <h3 className="text-lg font-semibold glass-text-strong flex items-center space-x-2">
                    <span>この日の支出</span>
                    <span className="text-sm font-normal glass-text">
                      (
                      {
                        expenses.filter(
                          expense =>
                        ...
                        >
                          <div className="flex-1 sm:mb-0">
                            <div className="flex flex-col sm:flex-row sm:items-center space-y-1 sm:space-y-0 sm:space-x-3">
                              ...
                              <span
                                className={`text-xl font-bold ${expense.amount < 0 ? 'text-emerald-600' : 'text-red-500'}`}
                              >
                                {expense.amount < 0
